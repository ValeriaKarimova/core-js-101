...
function extractNameFromTemplate(value) {
  ...
  const newArr = ArrFromStr.splice(1, 2);
...
function getRectangleString(width, height) {
  const times = width - 2;
  const verticalTimes = height - 2;
  const dash = '─';
  const space = ' ';
  const verticalDash = '│';
  const spaces = space.repeat(times);
  const line = `${verticalDash}${spaces}${verticalDash}\n`;
  const lines = line.repeat(verticalTimes);
  const widthLine = dash.repeat(times);
  const rectangle = `┌${widthLine}┐\n${lines}└${widthLine}┘\n`;
  return rectangle;
}
